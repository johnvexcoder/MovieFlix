...
export async function POST(request: Request) { try {
  ...
  const b=await request.json(); const username=String(b.username||"").trim(), fullName=String(b.fullName||"").trim(), dateOfBirth=String(b.dateOfBirth||""), contactNumber=String(b.contactNumber||"").trim(), email=String(b.email||"").trim().toLowerCase(), password=String(b.password||"");
  ...
  const [duplicate]=await db.select({id:accounts.id}).from(accounts).where(or(eq(accounts.username,username),eq(accounts.email,email))).limit(1);
