...
export function PaymentSubmissionsAdmin() {
  ...
  const shown = useMemo(() => items.filter((item) => `${item.username} ${item.email} ${item.planName} ${item.status} ${item.providerPaymentId} ${item.promoCode} ${item.provider} ${item.method} ${item.createdAt} ${new Date(item.createdAt).toLocaleString()} ${money(item.finalAmountMinor)}`.toLowerCase().includes(query.toLowerCase())), [items, query]);
  ...
  return <section className="glass-panel min-w-0 rounded-3xl border border-white/10 p-4 sm:p-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="flex items-center gap-2 text-lg font-bold"><CreditCard className="h-5 w-5 text-cyan-300"/>Automated payments</h2><p className="text-xs text-slate-400">Statuses refresh automatically. Access activates only through verified payment events.</p></div><div className="relative w-full sm:w-64"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-500"/><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search customer, status, date, method…" className="pl-9"/></div></div>
